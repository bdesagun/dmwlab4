...
def to_df_flg(url):
    """Update the SQL table if the html has been pickled to DataFrame."""
    server = ''
    db = ''
    account = ''
    password = ''
...
def build_df_pkl():
    """Build a DataFrame pickle for every html file."""
    server = ''
    db = ''
    account = ''
    password = ''
    ...
    # u=url
    df_cols = ['date_yyyymmdd', 'division', 'case no', 'case title', 
            'petitioner',
    ...
    for i, r in df_cases.iterrows():
        ...
        if len(full_body) > 1:  # merging the body in ca
        ...
        disposition = re.findall(
            'affirm|affirms|affirmed|deny|denies|denied|dismiss|dismissed'
            '|grant'
            '|granted|modify|modified|modification|remand|remanded|refer'
            '|referred'
        ...
        try:
            ...
            granted, denied, dismissed, remanded_referred = None, None,\
                 None, None
...
def get_cases_from_pkl(path, pattern_filter):
    """Get DataFrame pickle files that matches regex pattern."""
    out = []
...
def main():
    """Build the final pickle file for Labor Law related cases."""

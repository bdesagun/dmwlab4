...
# u=url
df_cols = ['date_yyyymmdd', 'division', 'case no', 'case title', 'petitioner', 'respondent', 'ponente', 'body', 'dispositive portion', 'disposition', 'granted', 'denied', 'dismissed', 'remanded_referred', 'other justices', 'law', 'provision', 'footnote', 'url']
...
for i,r in df_cases.iterrows():
    ...
    case_details = re.sub(r'(?i)\s*D\s*E\s*C\s*I\s*S\s*I\s*O\s*N.*|'
                        '\s*R\s*E\s*S\s*O\s*L\s*U\s*T\s*I\s*O\s*N.*', '', case_details)
    case_details = re.sub(r'\s+', ' ', case_details) #removing excess whitespaces
    case_details = re.sub(r', V\.', ', VS.', case_details) #removing standardizing v and vs
    ...
    so_ordered = full_body.rfind('so ordered') #see if the next word is a SC justice
    # case_soup.select('hr')
    #Getting the main body based on the marker
    main_body = full_body[2:so_ordered+11] #we start at 2 to remove ['


    # r'family code|domestic adoption|intercountry adoption|inter-country adoption'
    ...
    if dispositive.find('wherefore') != -1:
        ...
        dispositive = dispositive[dispositive.find('in view of the foregoing premises'):]
    elif dispositive.find('considering the foregoing disquisitions') != -1:
        dispositive = dispositive[dispositive.find('considering the foregoing disquisitions'):]
    elif main_body[-1000:].find('accordingly, the petition is'):
        dispositive = dispositive[dispositive.find('accordingly, the petition is'):]
    elif main_body[-1000:].find('accordingly, in line with the foregoing disquisition, the petition is'):
        dispositive = dispositive[dispositive.find('accordingly, in line with the foregoing disquisition, the petition is'):]
    elif main_body[-1000:].find('in light of the all the foregoing'):
        dispositive = dispositive[dispositive.find('in light of the all the foregoing,'):]
    elif main_body[-1000:].find('considering the foregoing'):
        dispositive = dispositive[dispositive.find('considering the foregoing,'):]

    #appeal|petition|decision|court of appeals|\w+ trial court
    #Getting the disposition based on the marker
    disposition = re.findall('affirm|affirms|affirmed|deny|denies|denied|dismiss|dismissed|grant|granted|modify|modified|modification|remand|remanded|refer|referred|set aside|sets aside|reverses|reversed|reinstate|reinstated', dispositive)
    disposition_kw = {'affirm': 'affirmed', 'affirms':'affirmed', 'deny': 'denied', 'denies':'denied', 'dismiss': 'dismissed','grant': 'granted', 'modify': 'modified', 'modification': 'modified', 'remand': 'remanded', 'refer':'referred', 'reverses':'reversed','reinstate':'reinstated', 'sets aside': 'set aside'}
    disposition = [disposition_kw[d] if d in disposition_kw.keys() else d for d in disposition]

    #Granted, Denied
    try:
        if disposition[0] == 'dismissed':
            granted, denied, dismissed, remanded_referred = False, False, True, False
        if disposition[0] == 'granted' or disposition[0] == 'set aside':
            granted, denied, dismissed, remanded_referred = True, False, False, False
        elif disposition[0] == 'denied' or disposition[0] == 'deny' or disposition[0] == 'denies' or disposition[0] == 'affirmed':
            granted, denied, dismissed, remanded_referred = False, True, False, False
            
    #             if search(disposition, 'referred') or search(disposition, 'remanded'):
    #                 remanded_referred = True
    ...
    provision = Counter(re.findall('(?i) article \d+| art. \d+| art \d+|'
                                    ' section \d+| sec. \d+| sec \d+', main_body))
    ...
    case_text_df = pd.DataFrame(dict(zip(df_cols, [date, division, case_no, case_title, petitioner, respondent, ponente, main_body, dispositive, [disposition], granted, denied, dismissed, remanded_referred, other_j,[law], [provision], footnote_dump, u])))
    case_text_df.to_pickle(f"../{url.replace('.html','.pkl')}")
    if not (ctr % 200):
        display(case_text_df)
    to_df_flg(url)
    # all_text_df = pd.concat([all_text_df, case_text_df]).reset_index(drop=True)
    ctr +=1 
